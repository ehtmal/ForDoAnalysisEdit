...
### (参考) 開発版を利用する
アプリケーションとして利用せずに、開発中のアプリケーションを利用する手順です。
...
なお、開発版、アプリケーション版ともに同じアプリケーションフォルダを利用するため、作成したコンテンツ等は共有可能です。
...
#### 3. API キーの設定
2. で選択した LLM の API キーを設定します。

### （任意） 初期設定
オンボーディングでの設定以外にも以下の設定項目があります。右上の設定ボタンから変更可能です。
...
#### 新規プロジェクトの作成
ダッシュボード画面の「新規作成」ボタンをクリックして、新規プロジェクトを作成します。
...
3.  **出力設定**: 出力形式（動画、音声、PDF）を指定し、コンテンツ生成します。
4.  **成果物**: 生成されたコンテンツを確認します。

#### 1. AI チャット
- 設定したLLMと対話しながらコンテンツを作成します。
- URLを入力して、Webページの内容を読み込ませることも可能です。
- `EXA API` を活用すると、`yyyy-mm-dd の主要なAIニュースを調査して` といった検索が必要な依頼も可能です。
- スクリプトを作成するには、テンプレートを選んで「入力欄にコピー」し、内容を修正して送信ボタンを押してください。

#### 2. スクリプト編集
6つのエリアで構成されており、タブでエリアを切り替えます。

- Text
  - 音声生成のベースとなる文章を編集します。
  - 文章から音声への変換、翻訳もここで行います。
- YAML/JSON
  - スクリプト全体をYAMLまたはJSON形式で直接編集します。
- Media  
  - **画像/動画**: 画像/動画は以下の方法で取得できます。
    - プロンプトによる生成
      - 画像だけ生成
      - 画像を参考に動画生成
      - リップシンク（音声に合わせて口元を動かす）
      - `Ref` タブで設定した参照画像を利用する
    - ファイルのドラッグ&ドロップ
    - URLを入力して取得
  - **スライド**: 
...
- Style
  - 生成するコンテンツに関する設定です。
    ...
    - Movie Parameters: 動画生成に必要な情報
      - どのプロバイダ（Replicate、Googleなど）の、どのモデルを利用するか
    - Text Slide Parameters: 「テキストスライド」の CSSスタイル
    - Audio Parameters: 音声合成に必要な情報
      - ビート間の間（ま）など
      - BGM 変更
    - Caption Parameters
      - どの言語でキャプションを表示するか
